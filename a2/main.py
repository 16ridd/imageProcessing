...
def compute():
  ...
  dc = 0 #true max value
  max1 = 0 #max value that we are looking for
  ...
  #creates a blank image to store the high magnitude pixels in
  ...
  #find cutoff value and creates the grid image and puts high magnitude pixels into the blank image
  cutoff = 0.4 * max1
  for i in range (0, height):
      for j in range (0, width):
          if mags[i,j] > cutoff:
  ...
  angle = []
  check = [[]*2 for x in xrange(2)]

  
  #adjusts pixels on the left side of the image so that the angle and distance can be properally calculated        
  for i in range (0,(height/2)):
      for j in range (0, width):
          if (blank[i,j] == 1 and j >= width/2):
              k = j - width
              blank[i,j] = blank[i,k]
 
  #finds the angle and distance of each point
  for i in range (0,(height/2)):
      for j in range (-width/2, width/2):
          if blank[i,j] == 1:
              angle.append(math.degrees(math.atan2(i,j)))
              check[1].append(math.degrees(math.atan2(i,j)))
              check[0].append(math.sqrt(j*j + i*i))
  
 
  #gets rid of the first 3 pixels that a found right at the origin.  
  angle = angle[3:]
 
  # sets all angles that are 180 to 0, a fix for when the picture is square to the image and
  # allows for proper angle calcualtions  
  ...
  #find where to split for the two line
  ...
  #compairs line1 and check to find when angles are the same to 
  #put corisponding distance into distance list for line1 
  for i in range (0, len(check[1])):
      for j in range (0, len(line1)):
          if (line1[j]) == (check[1][i]):
              distance1.append(check[0][i])
            
              
  for i in range(0, len(check[1])):
      for j in range(0, len(line2)):
          if (line2[j]) == (check[1][i]):
              distance2.append(check[0][i])
...
##### for finding the minimum value there was a value that was closer than 40 that was found but 
##### it was ignored by making the mimimum distance allowed 10 because the proofs answers showed 40 as the mimimum distance
  #finds minumum distance for line1  
  for i in range(0,len(distance1)):
      if distance1[i] > 10:
          minDistance1 = distance1[i]
          break
      
  #finds minumum distance for line2      
  for i in range(0,len(distance2)):
      if distance2[i] > 10:
          minDistance2 = distance2[i]
          break    
  
  
  #finds average of all angles from lines
  line1average = np.average(line1)
  line2average = np.average(line2)
  
  #output the angles and distances
  lines = [[line1average,minDistance1],[line2average,minDistance2]]
  ...
  #removes all pixels from final image when corrisponding pixel in the grid image brightness is greater than 16  
  for i in range (0, height):
      for j in range (0, width):
          if gridImage[i,j] > 16:
              resultImage[i,j] = 0
